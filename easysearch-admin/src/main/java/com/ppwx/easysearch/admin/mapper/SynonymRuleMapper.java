 ...
 */
public interface SynonymRuleMapper extends BaseMapper<SynonymRuleDO> {
    @Delete("DELETE FROM qp_rule_synonym WHERE resource_set_id = #{resourceSetId}")
    int deleteByResourceSetId(@Param("resourceSetId") Long resourceSetId);

    @Select("SELECT COUNT(*) FROM qp_rule_synonym WHERE resource_set_id = #{resourceSetId}")
