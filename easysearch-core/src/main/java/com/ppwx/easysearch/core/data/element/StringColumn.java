...
import com.ppwx.easysearch.core.data.element.ColumnCast;
import com.ppwx.easysearch.core.util.OverFlowUtil;
...
public class StringColumn extends Column {
	...
	private void validateDoubleSpecific(final String data) {
		if ("NaN".equals(data) || "Infinity".equals(data)
			...
			throw new IllegalArgumentException(String.format("String[\"%s\"]属于Double特殊类型，不能转为其他类型 .", data));
		}
	...
	public Long asLong() {
		...
		try {
			...
			throw new IllegalArgumentException(String.format("String[\"%s\"]不能转为Long .", this.asString()));
		}
	}
	...
	public Boolean asBoolean() {
		...
		throw new IllegalArgumentException(String.format("String[\"%s\"]不能转为Bool .", this.asString()));
	}
	...
	public Date asDate() {
		try {
			...
			throw new IllegalArgumentException(String.format("String[\"%s\"]不能转为Date .", this.asString()));
		}
	}
	...
	public byte[] asBytes() {
		try {
			...
			throw new IllegalArgumentException(String.format("String[\"%s\"]不能转为Bytes .", this.asString()));
		}
	}
}
