package com.ppwx.easysearch.core.conf;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.serializer.SerializerFeature;
import com.ppwx.easysearch.core.util.StrUtil;
...
public class Configuration {
	...
	public static Configuration from(File file) {
		try {
			...
			throw new IllegalArgumentException(String.format("配置信息错误，您提供的配置文件[%s]不存在. 请检查您的配置文件.", file.getAbsolutePath()));
		} catch (IOException e) {
			throw new IllegalArgumentException(String.format("配置信息错误. 您提供配置文件[%s]读取失败，错误原因: %s. 请检查您的配置文件的权限设置.",
	...
	public static Configuration from(InputStream is) {
		try {
			...
			throw new IllegalArgumentException(String.format("请检查您的配置文件. 您提供的配置文件读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", e));
		}
	}
	...
	public String getNecessaryValue(String key) {
		...
		if (StringUtils.isBlank(value)) {
			throw new IllegalArgumentException(String.format("您提供配置文件有误，[%s]是必填参数，不允许为空或者留白 .", key));
		}
	...
	public Character getChar(final String path) {
		...
		try {
			...
			throw new IllegalArgumentException(String.format("任务读取配置文件出错. 因为配置文件路径[%s] 值非法，期望是字符类型: %s. 请检查您的配置并作出修改.", path,
							e.getMessage()));
		}
	}
	...
	public Boolean getBool(final String path) {
		...
		if (null == result) {
			...
			throw new IllegalArgumentException(String.format("您提供的配置信息有误，因为从[%s]获取的值[%s]无法转换为bool类型. 请检查源表的配置并且做出相应的修改.",
							path, result));
		}
	...
	public Integer getInt(final String path) {
		...
		try {
			...
			throw new IllegalArgumentException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是整数类型: %s. 请检查您的配置并作出修改.", path,
							e.getMessage()));
		}
	}
	...
	public Double getDouble(final String path) {
		...
		try {
			...
			throw new IllegalArgumentException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是浮点类型: %s. 请检查您的配置并作出修改.", path,
							e.getMessage()));
		}
	}
	...
	public Object remove(final String path) {
		...
		if (null == result) {
			throw new IllegalArgumentException(String.format("配置文件对应Key[%s]并不存在.", path));
		}
	...
	private void setObject(final String path, final Object object) {
		...
		throw new IllegalArgumentException(String.format("值[%s]无法适配您提供[%s]!",
	...
	Object setObjectRecursive(Object current, final List<String> paths,
		...
		throw new IllegalArgumentException("该异常代表系统编程错误!");
	}
	...
	private static void checkJSON(final String json) {
		if (StringUtils.isBlank(json)) {
			throw new IllegalArgumentException("配置信息错误. 因为您提供的配置信息不是合法的JSON格式, JSON不能为空白. 请按照标准json格式提供配置信息. ");
		}
	}
	...
	private Configuration(final String json) {
		try {
			...
			throw new IllegalArgumentException(String.format("配置信息错误. 您提供的配置信息不是合法的JSON格式: %s . 请按照标准json格式提供配置信息. ", e.getMessage()));
		}
	}
