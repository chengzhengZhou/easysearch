...
import type { ResourceSet, Snapshot, SentenceRule, TermRule, DiffSummary, PageResult, InterventionMode } from '../types'

// ==================== 基础状态 ====================
const mode = ref<InterventionMode>('sentence')
...
const selection = useSelection<SentenceRule | TermRule>()
...
// ==================== 计算属性 ====================
const modeLabel = computed(() => mode.value === 'sentence' ? '整句干预' : '词表干预')
...
async function loadDiffSummary() {
  ...
  try {
    const res = await http.get(`/api/resource-sets/${resourceSetId.value}/diff-summary`, { params: { module: 'intervention' } })
...
function openAddModal() {
  if (!resourceSetId.value) return
  addForm.value = { sourceText: '', targetText: '', matchType: 'EXACT', priority: 0, enabled: 1 }
...
<template>
  <div class="qp-admin">
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ...
                <tr v-for="(r, idx) in pagination.pagedData.value" :key="(r as any).id" :class="{ selected: selection.isSelected((r as any).id) }">
                  <td>
                    <input
                      type="checkbox"
                      :disabled="!resourceSetId"
                      :checked="selection.isSelected((r as any).id)"
                      @change="selection.toggleOne((r as any).id, ($event.target as HTMLInputElement).checked)"
                    />
                  </td>
    ...
    <QpModal :open="comparePickerOpen" title="版本对比" icon="⚖" type="primary" @close="comparePickerOpen = false">
      <QpAlert type="info" icon="ℹ">选择两个版本进行差异对比，对比维度：<strong>{{ modeLabel }}</strong></QpAlert>
    ...
    <!-- 新增规则弹窗 -->
    <QpModal :open="addModalOpen" title="新增规则" icon="＋" type="primary" @close="addModalOpen = false">
      <QpAlert type="info" icon="📝">当前模式：<strong>{{ modeLabel }}</strong></QpAlert>
      <div class="form-grid">
