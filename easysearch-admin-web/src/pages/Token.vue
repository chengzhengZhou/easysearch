...
import type { ResourceSet, Snapshot, TokenRule, DiffSummary, PageResult } from '../types'

// ==================== 基础状态 ====================
const resourceSetId = ref<number | null>(null)
const searchInput = ref<string>('')
const natureFilter = ref<string>('')
...
const filteredRows = computed(() => {
  ...
  if (k) {
    result = result.filter((r) => {
      ...
      return w.includes(k) || n.includes(k)
    })
  }
  if (natureFilter.value) {
    result = result.filter((r) => r.nature === natureFilter.value)
  }
  if (enabledFilter.value !== '') {
    result = result.filter((r) => String(r.enabled) === enabledFilter.value)
  }
  return result
})

const pagination = usePagination({ data: filteredRows, defaultPageSize: 20 })
const selection = useSelection<TokenRule>()
...
const natureOptions = [
  ...
  { value: 'category', label: '类目词' },
  { value: 'feature', label: '功能词' },
  { value: 'spec', label: '规格词' },
  { value: 'tag', label: '标签词' },
  { value: 'os', label: '系统词' },
  { value: 'NN', label: '名词' },
  { value: 'VV', label: '动词' },
...
const addForm = ref<{ word: string; nature: string; frequency: number | null; enabled: number }>({
  word: '',
  nature: 'NN',
  frequency: null,
  enabled: 1,
...
async function loadRules() {
  ...
  const res = await http.get(`/api/resource-sets/${resourceSetId.value}/rules`, {
    params: { module: 'token', q: searchInput.value || undefined, page: 1, pageSize: 500 },
  })
  const pr = res.data?.data as PageResult<TokenRule>
...
async function loadDiffSummary() {
  ...
  try {
    const res = await http.get(`/api/resource-sets/${resourceSetId.value}/diff-summary`, { params: { module: 'token' } })
...
function openAddModal() {
  if (!resourceSetId.value) return
  addForm.value = { word: '', nature: 'NN', frequency: null, enabled: 1 }
  addModalOpen.value = true
}

async function submitAdd() {
  if (!resourceSetId.value) return
  const word = String(addForm.value.word ?? '').trim()
  if (!word) {
    showToast('词语必填')
    return
  }
  const payload = {
    word: word,
    nature: addForm.value.nature || 'NN',
...
async function removeRule(id: number) {
  if (!resourceSetId.value) return
  await http.delete(`/api/resource-sets/${resourceSetId.value}/rules/${id}`, { params: { module: 'token' } })
  auditLog.value.unshift(`${new Date().toLocaleString()} | 删除规则 id=${id}`)
  await loadRules()
  await loadDiffSummary()
}

async function updateRuleCell(rule: TokenRule) {
  if (!resourceSetId.value) return
  await http.put(`/api/resource-sets/${resourceSetId.value}/rules/${rule.id}`, rule, { params: { module: 'token' } })
  await loadDiffSummary()
}

function toggleEnabled(rule: TokenRule) {
...
async function batchDelete() {
  ...
  const ids = selection.getSelectedIds()
  if (!ids.length) return
  await http.post(`/api/resource-sets/${resourceSetId.value}/rules/batch-delete`, { ids }, { params: { module: 'token' } })
...
async function confirmCompare() {
  ...
  try {
    ...
    diffSummary.value = `${labelA} vs ${labelB}（分词词典）—— 新增 ${diffAdded.value.length} 条、删除 ${diffDeleted.value.length} 条、修改 ${diffModified.value.length} 条`
...
function getNatureLabel(nature: string): string {
  const opt = natureOptions.find((o) => o.value === nature)
  return opt ? opt.label : nature
}

// ==================== 监听器 ====================
watch(
  () => [pagination.pageSize.value, searchInput.value, natureFilter.value, enabledFilter.value],
  () => pagination.resetPage()
)
  ...
  <div class="qp-admin">
    ...
    <section class="layout two-col">
      <div>
        ...
        <div class="panel panel-main">
          <div class="panel-header">
            <h3>规则列表</h3>
            <span class="panel-badge">分词词典</span>
          </div>
          <div class="rule-toolbar">
            <input v-model="searchInput" class="input search-input" placeholder="🔍 搜索词语 / 词性..." />
            <select v-model="natureFilter" class="select filter-select">
              <option v-for="opt in natureOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
            </select>
            <select v-model="enabledFilter" class="select filter-select">
            ...
            <table>
              ...
              <tbody>
                ...
                <tr v-for="(r, idx) in pagination.pagedData.value" :key="r.id" :class="{ selected: selection.isSelected(r.id) }">
                  ...
                  <td>
                    <input class="input" :disabled="!resourceSetId" v-model="r.word" @blur="updateRuleCell(r)" />
                  </td>
                  <td>
                    <select class="select" :disabled="!resourceSetId" v-model="r.nature" @change="updateRuleCell(r)">
                      <option v-for="opt in natureOptions.filter(o => o.value)" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
                    </select>
                  </td>
                  <td>
                    <input class="input input-number" type="number" :disabled="!resourceSetId" v-model.number="r.frequency" @blur="updateRuleCell(r)" placeholder="-" />
                  </td>
                  <td class="actions">
                    <button
      ...
      <div class="side-panels">
        <div class="panel panel-side">
          ...
          <div class="validate-box" :class="{ 'validate-ok': validateSummary.includes('ok') || validateSummary.includes('通过'), 'validate-fail': validateSummary.includes('失败') || validateSummary.includes('错误') }">
            {{ validateSummary }}
          </div>
        </div>
    ...
    <QpModal :open="publishConfirmOpen" title="确认发布" icon="🚀" type="primary" @close="publishConfirmOpen = false">
      ...
      <div class="validate-status" :class="{ 
        ...
        'status-ok': publishValidateSummary.includes('ok') || publishValidateSummary.includes('OK'),
    ...
    <QpModal :open="comparePickerOpen" title="版本对比" icon="⚖" type="primary" @close="comparePickerOpen = false">
      <QpAlert type="info" icon="ℹ">选择两个版本进行差异对比，对比维度：<strong>分词词典</strong></QpAlert>
            ...
            <table class="diff-table">
              ...
              <tbody>
                <tr class="diff-row-before">
                  ...
                  <td :class="{ 'diff-changed': m.before?.word !== m.after?.word }">{{ m.before?.word }}</td>
                  <td :class="{ 'diff-changed': m.before?.nature !== m.after?.nature }">{{ getNatureLabel(m.before?.nature ?? '') }}</td>
                  <td :class="{ 'diff-changed': m.before?.frequency !== m.after?.frequency }">{{ m.before?.frequency ?? '-' }}</td>
                  <td :class="{ 'diff-changed': m.before?.enabled !== m.after?.enabled }">{{ m.before?.enabled ? '启用' : '停用' }}</td>
                </tr>
                <tr class="diff-row-after">
                  <td class="diff-label">新</td>
                  <td :class="{ 'diff-changed': m.before?.word !== m.after?.word }">{{ m.after?.word }}</td>
                  <td :class="{ 'diff-changed': m.before?.nature !== m.after?.nature }">{{ getNatureLabel(m.after?.nature ?? '') }}</td>
                  <td :class="{ 'diff-changed': m.before?.frequency !== m.after?.frequency }">{{ m.after?.frequency ?? '-' }}</td>
                  <td :class="{ 'diff-changed': m.before?.enabled !== m.after?.enabled }">{{ m.after?.enabled ? '启用' : '停用' }}</td>
                </tr>
              </tbody>
            </table>
          </div>
    ...
    <!-- 新增规则弹窗 -->
    <QpModal :open="addModalOpen" title="新增分词词典规则" icon="＋" type="primary" @close="addModalOpen = false">
      <QpAlert type="info" icon="📝">添加自定义分词词条，支持配置词性和词频。</QpAlert>
      <div class="form-grid">
        <div class="form-field form-field-full">
          <label class="form-label">词语 <span class="form-required">*</span></label>
          <input v-model="addForm.word" class="input input-full" placeholder="输入词语" />
        </div>
        <div class="form-field">
          <label class="form-label">词性</label>
          <select v-model="addForm.nature" class="select select-full">
            <option v-for="opt in natureOptions.filter(o => o.value)" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
          </select>
        </div>
        <div class="form-field">
          <label class="form-label">词频 <span class="form-optional">（可选）</span></label>
          <input v-model.number="addForm.frequency" class="input input-full" type="number" placeholder="默认无" />
        </div>
        <div class="form-field">
