...
public class CosineSimilarity implements WordSimilarityScore<Double>, SequenceSimilarityScore<Double> {
    ...
    public Double apply(Collection<String> var1, Collection<String> var2) {
        if (var1 != null && var2 != null) {
            Map<CharSequence, Long> leftVector = var1.stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
            Map<CharSequence, Long> rightVector = var2.stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    ...
    @Override
    public Double apply(CharSequence var1, CharSequence var2, Filter<Character> charFilter) {
        if (var1 != null && var2 != null) {
            Map<CharSequence, Long> leftVector = new HashMap<>(var1.length());
            for (int i = 0; i < var1.length(); i++) {
                if (charFilter == null || !charFilter.accept(var1.charAt(i))) {
            ...
            for (int i = 0; i < var2.length(); i++) {
                if (charFilter == null || !charFilter.accept(var2.charAt(i))) {
